...
- **§7's three NaN fixes need to reach this round too** (flagged by the user, not yet done): §7's `_ReLUFix` is already applied to `efficientnet_unet.py`'s custom decoder blocks (the pretrained encoder itself is out of reach — still Keras Applications' own ReLU/Swish, see §7's caveat), and §7.2's `Adam(clipvalue=...)` already covers it for free (`compile_model` applies it unconditionally, regardless of `uses_advanced_loss`). What's **not yet done**: `022_training_efficientnet.ipynb` still trains `efficientnet_unet`/`efficientnet_unet_nll` in an in-process loop, not via §7.1's `scripts/train_single.py` subprocess-per-architecture pattern — needs the same restructuring `020`/`021`/`023` got, including a `--nll` path that can pass `freeze_encoder`/other kwargs through `--kwargs` for the two-phase fine-tuning checkpoint. Do this as part of implementing Round 2 (or before, if `022` is run under the pre-Round-2 recipe and hits the same GPU-state-accumulation NaN pattern `020` did).
